...
func (m appModel) keybar() string {
	...
	case focusDetail:
		return "←/→ panels · ↑/↓ scroll · b databases · K SSH keys · c cron · esc subscriptions · ? help"
	case focusLogs:
		return "←/→ panels · ↑/↓ scroll · l/L logs · esc subscriptions · ? help"
	default:
		return "←/→ panels · ↑/↓ scroll · h health · R reconcile · K SSH keys · c cron · esc subscriptions · ? help"
	}
}
