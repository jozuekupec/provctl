...
func NewRootCommand() *cobra.Command {
	...
	root.RunE = func(command *cobra.Command, _ []string) error {
		...
		backupWriteRuntime, err := service.NewProductionBackupRuntime(context.Background(), cfg)
		if err != nil {
			return fmt.Errorf("open backup mutation TUI state: %w", err)
		}
		defer backupWriteRuntime.Close()
