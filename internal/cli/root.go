...
func NewRootCommand() *cobra.Command {
	...
	root.RunE = func(command *cobra.Command, _ []string) error {
		...
		databaseWriteRuntime, err := service.NewProductionDatabaseRuntime(context.Background(), cfg)
		if err != nil {
			return fmt.Errorf("open database mutation TUI state: %w", err)
		}
		defer databaseWriteRuntime.Close()
		...
		sshWriteRuntime, err := service.NewProductionSSHRuntime(context.Background())
		if err != nil {
			return fmt.Errorf("open SSH mutation TUI state: %w", err)
		}
		defer sshWriteRuntime.Close()
