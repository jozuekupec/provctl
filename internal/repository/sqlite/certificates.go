...
func (repository *Repository) CreateCertificate(ctx context.Context, certificate domain.Certificate) (int64, error) {
	...
	result, err := repository.DB.ExecContext(ctx, `INSERT INTO certificates (subscription_id, website_id, lineage, primary_domain, sans, issuer, managed, not_before, not_after, last_checked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, certificate.SubscriptionID, nullableInt64(certificate.WebsiteID), certificate.Lineage, certificate.PrimaryDomain, string(sans), nullable(certificate.Issuer), certificate.Managed, nullableTime(certificate.NotBefore), nullableTime(certificate.NotAfter), nullableTime(certificate.LastCheckedAt))
	if err != nil {
...
func (repository *Repository) ListCertificates(ctx context.Context, subscriptionID int64) ([]domain.Certificate, error) {
	rows, err := repository.DB.QueryContext(ctx, `SELECT id, subscription_id, COALESCE(website_id, 0), lineage, primary_domain, sans, COALESCE(issuer, ''), managed, COALESCE(not_before, ''), COALESCE(not_after, ''), COALESCE(last_checked_at, '') FROM certificates WHERE subscription_id = ? ORDER BY lineage`, subscriptionID)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&certificate.ID, &certificate.SubscriptionID, &certificate.WebsiteID, &certificate.Lineage, &certificate.PrimaryDomain, &sans, &certificate.Issuer, &certificate.Managed, &notBefore, &notAfter, &checked); err != nil {
...
func (repository *Repository) CertificateByWebsite(ctx context.Context, websiteID int64) (domain.Certificate, error) {
	row := repository.DB.QueryRowContext(ctx, `SELECT id, subscription_id, COALESCE(website_id, 0), lineage, primary_domain, sans, COALESCE(issuer, ''), managed, COALESCE(not_before, ''), COALESCE(not_after, ''), COALESCE(last_checked_at, '') FROM certificates WHERE website_id = ?`, websiteID)
...
func (repository *Repository) CertificateByLineage(ctx context.Context, lineage string) (domain.Certificate, error) {
	row := repository.DB.QueryRowContext(ctx, `SELECT id, subscription_id, COALESCE(website_id, 0), lineage, primary_domain, sans, COALESCE(issuer, ''), managed, COALESCE(not_before, ''), COALESCE(not_after, ''), COALESCE(last_checked_at, '') FROM certificates WHERE lineage = ?`, lineage)
	return scanCertificate(row)
}
...
func scanCertificate(row *sql.Row) (domain.Certificate, error) {
	var certificate domain.Certificate
	var sans, notBefore, notAfter, checked string
	if err := row.Scan(&certificate.ID, &certificate.SubscriptionID, &certificate.WebsiteID, &certificate.Lineage, &certificate.PrimaryDomain, &sans, &certificate.Issuer, &certificate.Managed, &notBefore, &notAfter, &checked); err != nil {
