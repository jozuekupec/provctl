...
func TestApplyMigrations_RejectsNewerSchema(t *testing.T) {
	...
	if _, err := database.Exec(`INSERT INTO schema_migrations VALUES (4, '2026-01-01T00:00:00Z')`); err != nil {
...
func TestInspectSchema_ReadsCurrentVersion(t *testing.T) {
	...
	if info.Current != 3 || info.Latest != 3 {
		t.Errorf("InspectSchema() = %#v, want current and latest 3", info)
	}
}
...
func TestApplyMigrations_PreservesBackupsFromSchemaOne(t *testing.T) {
	...
	var version int
	if err := database.QueryRow(`SELECT MAX(version) FROM schema_migrations`).Scan(&version); err != nil || version != 3 {
